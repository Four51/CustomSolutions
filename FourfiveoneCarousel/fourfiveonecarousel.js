four51.app.directive('customSlide', ['$compile','$http', '$templateCache',
    function($compile, $http, $templateCache) {
        var getTemplate = function(tn){
            var templateLoader,
                baseUrl = 'partials/SlideTemplates/',
                tn = tn + '.html',
                templateUrl = baseUrl + tn;

            templateLoader = $http.get(templateUrl, {cache: $templateCache});
            return templateLoader;
        };
        var linker = function(scope, element, attrs){
            var loader = getTemplate(scope.templateName);
            var backgroundcolorprop, backgroundimageprop;
            scope.bgColor ? (backgroundcolorprop = "background-color:"+ scope.bgColor + ";") : (backgroundcolorprop = '');
            scope.bgImg ? (backgroundimageprop = "background-image:url(\'" + scope.bgImg + "\');") : (backgroundimageprop = '');
            var slidestyle = "style=\"" + backgroundcolorprop + backgroundimageprop + "\"",
                slidebegin = "<li class=\"rn-carousel-slide\" " + slidestyle + "><div ng-class=\"{'rn-slide-contained-content':contain}\">",
                slideend = "</div></li>";
            var promise = loader.success(function(html){
                html = slidebegin + html + slideend;
                element.html(html);
...
//DIRECTIVES PULLED FROM ANGULAR-CAROUSEL :: https://github.com/revolunet/angular-carousel/blob/master/dist/angular-carousel.js

four51.app.directive('rnCarouselAutoSlide', ['$timeout', function($timeout) {
    return {
        restrict: 'A',
...
four51.app.directive('rnCarouselControls', [function() {
    return {
        restrict: 'A',
        replace: true,
        scope: {
...
four51.app.directive('rnCarouselIndicators', [function() {
    return {
        restrict: 'A',
        replace: true,
        scope: {
...
four51.app.directive('rnCarousel', ['$swipe', '$window', '$document', '$parse', '$compile', '$rootScope', function($swipe, $window, $document, $parse, $compile, $rootScope) {
