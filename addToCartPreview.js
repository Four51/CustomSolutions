/*
 ...
 The following will need to be added to the product detail template in place of the "AddToOrder" button
 <addtocartpreview lineitem="LineItem" errors="lineItemErrors" user="user"></addtocartpreview>
 */


four51.app.factory('addToCartPreview', ['$resource', '$451', 'User', 'Order', function($resource, $451, User, Order) {
...
four51.app.directive('addtocartpreview', function() {
    var obj = {
        restrict: 'E',
        scope: {
            lineitem : '=',
        ...
        template:'  <button class="btn btn-info btn-block btn-lg" type="button" ng-click="saveVariant(lineitem)"  ng-disabled="errors.length > 0">' +
...
four51.app.controller('addToCartPreviewCtrl', ['$scope', '$routeParams', '$route', '$location', '$451', 'Order', 'addToCartPreview',
    function ($scope, $routeParams, $route, $location, $451, Order, addToCartPreview) {
        $scope.list = [];
        ...
        $scope.addListToCart = function(list) {
            ...
            addToCartPreview.addListToCart(list, $scope.$parent.currentOrder, $scope.user, function(data) {
                    $scope.addListToCartIndicator = false;
                    $location.path('cart');
                },
                function(ex) {
