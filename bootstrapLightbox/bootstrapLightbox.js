...
angular.module('bootstrapLightbox').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('lightbox.html',
    "<div class=modal-body ng-swipe-left=Lightbox.nextImage() ng-swipe-right=Lightbox.prevImage()><div class=lightbox-nav><button class=close aria-hidden=true ng-click=$dismiss()>×</button><div class=btn-group><a class=\"btn btn-xs btn-default\" ng-click=Lightbox.prevImage()>‹ Previous</a><a class=\"btn btn-xs btn-default\" ng-click=Lightbox.nextImage()>Next ›</a></div></div><div class=lightbox-image-container><div class=lightbox-image-caption><span>{{Lightbox.image.caption}}</span></div><img lightbox-src={{Lightbox.image.url}} alt=\"\"></div></div>"
  );
...
angular.module('bootstrapLightbox').provider('Lightbox', function () {
  ...
  this.$get = function ($document, $modal, $timeout, ImageLoader) {
	...
	// change Image (working)
		Lightbox.changeImage = function (newIndex) {
      if (!(newIndex in images) || !('url' in images[newIndex])) {
        throw 'Invalid image.';
      }
